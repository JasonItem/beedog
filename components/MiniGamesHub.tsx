...
import { Gamepad2, Trophy, ArrowLeft, Star, Rocket, Pickaxe, Shield, CarFront, Activity, Volleyball, ChevronsUp, Layers, Scissors, CircleDashed, Grid3X3, Users, TrendingUp, Anchor, Maximize, Minimize2, Volume2, VolumeX, BarChart2, Ticket, Coins, Utensils, Info, Play, Flame, Zap, MessageSquare, Send, ThumbsUp, Crown, AlertTriangle, AlertCircle, CheckCircle2, CheckCircle2 as CheckCircleIcon, ChevronDown, DollarSign, Bike, Anchor as AnchorIcon, Sprout, Footprints, MousePointer2, Shuffle, ArrowUpCircle, Sword, Circle } from 'lucide-react';
...
import { Gomoku } from './games/Gomoku'; // New Game
...
import { getGomokuTierInfo } from '../services/gomokuService';
...
export const GAMES: GameConfig[] = [
  ...
  { id: 'gomoku', nameKey: 'game.gomoku.name', descKey: 'game.gomoku.desc', howToPlay: 'Standard Gomoku rules. Connect 5 pieces horizontally, vertically or diagonally.', color: 'from-blue-600 to-indigo-800', icon: Circle, tags: ['PvP', 'Strategy'], isHoneyGame: true },
...
export const MiniGamesHub: React.FC<MiniGamesHubProps> = ({ onLoginRequest }) => {
  ...
  const isRewardClaimed = (gameId: string) => userProfile?.dailyGameRewards?.[gameId] === todayStr;
  ...
  const handleGameStart = () => {
      ...
      if (fullscreenRef.current) {
          fullscreenRef.current.requestFullscreen().catch(e => console.log("FS error", e));
      }
  };
    ...
    if (selectedGameConfig) {
      ...
      if (user) {
          try {
              ...
              if (globalRes.success) earnedTotal += globalRes.earned;
              const gameRes = await claimPerGameDailyReward(user.uid, selectedGameConfig.id);
              if (gameRes.success) earnedTotal += gameRes.earned;
              if (earnedTotal > 0) {
                  showNotif(`🎉 +${earnedTotal} Honey`, 'success');
                  await refreshProfile();
              }
          } catch (e) { console.error("Mission failed", e); }
      }
    }
  };
  ...
  const handleSubmitReview = async () => {
      ...
      } catch (e) { showNotif("Failed to post review", 'error'); } 
  ...
  const renderGame = () => {
    switch (selectedGameConfig.id) {
      ...
      case 'gomoku': return <Gomoku userProfile={userProfile} onGameOver={handleGameOver} />;
  ...
  if (isPlaying) {
      return (
        <div ref={fullscreenRef} className="fixed inset-0 z-[200] bg-[#050505] flex flex-col animate-in fade-in zoom-in duration-300">
  ...
  const isPvP = selectedGameConfig.id === 'bee_chess' || selectedGameConfig.id === 'gomoku';
    ...
    <div className="min-h-screen pt-24 pb-12 bg-neutral-50 dark:bg-[#050505]">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="lg:hidden mb-6 relative z-30">
            <button onClick={() => setIsGameSelectorOpen(!isGameSelectorOpen)} className="w-full bg-white dark:bg-[#161616] p-4 rounded-2xl shadow-lg border border-neutral-200 dark:border-[#333] flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-brand-yellow text-black`}><selectedGameConfig.icon size={20} /></div>
                    <div className="text-left"><div className="text-xs text-neutral-500 font-bold uppercase tracking-wider">{t('games.select')}</div><div className="font-bold dark:text-white">{t(selectedGameConfig.nameKey)}</div></div>
                </div>
            ...
            {isGameSelectorOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#161616] rounded-2xl shadow-2xl border border-neutral-200 dark:border-[#333] overflow-hidden max-h-[60vh] overflow-y-auto custom-scrollbar animate-in slide-in-from-top-2">
                    {GAMES.map(game => (
                        <button key={game.id} onClick={() => setSelectedGameConfig(game)} className={`w-full p-3 flex items-center gap-3 border-b border-neutral-100 dark:border-[#222] last:border-0 active:bg-neutral-100 dark:active:bg-[#222] transition-colors ${selectedGameConfig.id === game.id ? 'bg-brand-yellow/10 dark:bg-brand-yellow/20' : ''}`}>
                            <div className="w-8 h-8 rounded-lg bg-neutral-100 dark:bg-[#333] flex items-center justify-center text-neutral-500"><game.icon size={16}/></div>
                            <div className="flex-1 text-left flex justify-between items-center"><div><div className={`font-bold text-sm ${selectedGameConfig.id === game.id ? 'text-brand-yellow' : 'dark:text-white'}`}>{t(game.nameKey)}</div>{game.isHoneyGame && <div className="text-[10px] text-orange-500 font-bold flex items-center gap-1"><Zap size={8} className="fill-current"/> {t('games.earn_honey')}</div>}</div>{isRewardClaimed(game.id) && <CheckCircleIcon size={16} className="text-green-500 fill-green-500/20"/>}</div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-start relative">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
                <div className={`relative rounded-[2.5rem] overflow-hidden p-8 md:p-12 text-white shadow-2xl transition-all duration-500 bg-gradient-to-br ${selectedGameConfig.color}`}>
                    ...
                    {selectedGameConfig.isHoneyGame && <div className="absolute top-6 right-6 bg-yellow-400 text-black px-4 py-1.5 rounded-full font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg animate-pulse"><Zap size={14} className="fill-black"/> {t('games.earn_honey')}</div>}
                    <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start">
                        <div className="w-24 h-24 md:w-32 md:h-32 bg-white/20 backdrop-blur-md rounded-3xl flex items-center justify-center shadow-inner border border-white/30 shrink-0"><selectedGameConfig.icon size={64} className="text-white drop-shadow-md" /></div>
                        <div className="flex-1 text-center md:text-left space-y-4">
                            <div><h2 className="text-4xl md:text-5xl font-black mb-2 tracking-tight">{t(selectedGameConfig.nameKey)}</h2><div className="flex flex-wrap gap-2 justify-center md:justify-start">{selectedGameConfig.tags.map(tag => (<span key={tag} className="bg-black/20 px-3 py-1 rounded-lg text-xs font-bold backdrop-blur-sm border border-white/10">#{tag}</span>))}{avgRating > 0 && (<span className="bg-yellow-400 text-black px-3 py-1 rounded-lg text-xs font-black flex items-center gap-1 backdrop-blur-sm border border-white/20"><Star size={12} className="fill-black"/> {avgRating.toFixed(1)}</span>)}</div></div>
                            <p className="text-white/90 text-lg leading-relaxed font-medium max-w-xl">{t(selectedGameConfig.descKey)}</p>
                            <div className="flex gap-4 justify-center md:justify-start mt-2"><Button onClick={handleGameStart} className="bg-white text-black hover:bg-neutral-100 border-none px-10 py-4 text-xl font-black shadow-xl hover:shadow-2xl hover:scale-105 transition-all"><Play className="mr-2 fill-black" /> {t('games.play_now')}</Button></div>
                        </div>
                    </div>
                </div>

                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-white dark:bg-[#161616] rounded-3xl p-6 border border-neutral-200 dark:border-[#333] shadow-sm relative overflow-hidden group"><div className="absolute -bottom-6 -right-6 opacity-5 dark:opacity-10 transition-transform group-hover:scale-110"><selectedGameConfig.icon size={120} className="text-black dark:text-white"/></div><h3 className="font-bold text-lg dark:text-white mb-4 flex items-center gap-2"><Info size={20} className="text-blue-500"/> {t('games.how_to_play')}</h3><p className="text-neutral-600 dark:text-neutral-400 text-base leading-relaxed relative z-10 font-medium">{selectedGameConfig.howToPlay}</p></div>
                    <div className="bg-white dark:bg-[#161616] rounded-3xl p-6 border border-neutral-200 dark:border-[#333] shadow-sm flex flex-col flex-1 min-h-0">
                        <div className="flex bg-neutral-100 dark:bg-[#222] p-1 rounded-xl mb-4 shrink-0">
                            <button onClick={() => setActiveTab('LEADERBOARD')} className={`flex-1 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'LEADERBOARD' ? 'bg-white dark:bg-[#333] shadow-sm text-black dark:text-white' : 'text-neutral-500'}`}><Activity size={16}/> {t('games.leaderboard')}</button>
                            <button onClick={() => setActiveTab('REVIEWS')} className={`flex-1 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'REVIEWS' ? 'bg-white dark:bg-[#333] shadow-sm text-black dark:text-white' : 'text-neutral-500'}`}><MessageSquare size={16}/> {t('games.reviews')}</button>
                        </div>
                        {activeTab === 'LEADERBOARD' ? (
                            <><div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 mb-2 pr-1 h-[250px]">{loadingData ? (<div className="text-center text-neutral-400 py-8 text-sm">Loading...</div>) : leaderboard.length === 0 ? (<div className="text-center text-neutral-400 py-8 text-sm">No Records</div>) : (leaderboard.map((s, idx) => {
                                            const tier = selectedGameConfig.id === 'bee_chess' ? getTierInfo(s.score) : (selectedGameConfig.id === 'gomoku' ? getGomokuTierInfo(s.score) : null);
                                            return (<div key={idx} className="flex items-center justify-between text-sm p-3 hover:bg-neutral-50 dark:hover:bg-[#222] rounded-xl transition-colors"><div className="flex items-center gap-3"><span className={`w-6 text-center font-black ${idx < 3 ? 'text-yellow-500 text-lg' : 'text-neutral-400'}`}>{idx+1}</span><div className="w-8 h-8 rounded-full bg-neutral-200 dark:bg-[#333] overflow-hidden border border-neutral-100 dark:border-[#444] shrink-0">{s.avatarUrl ? <img src={s.avatarUrl} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-xs">🐶</div>}</div><div className="flex flex-col"><span className="font-bold dark:text-gray-200 truncate max-w-[100px]">{s.nickname}</span>{tier && (<span className={`text-[9px] font-black px-1 rounded-sm w-fit ${tier.bg} ${tier.color}`}>{tier.name} {tier.stars}★</span>)}</div></div><div className="flex flex-col items-end"><span className="font-mono font-black dark:text-white bg-neutral-100 dark:bg-[#333] px-2 py-1 rounded text-xs">{s.score}</span><span className="text-[8px] text-neutral-500 uppercase font-bold mt-1">{isPvP ? 'Points' : 'Score'}</span></div></div>);
                                        }))}</div>{userProfile && (<div className="pt-3 border-t border-neutral-100 dark:border-[#333] shrink-0"><div className="bg-brand-yellow/10 border border-brand-yellow/30 p-3 rounded-xl flex items-center justify-between"><div className="flex items-center gap-2"><Crown size={16} className="text-brand-yellow fill-brand-yellow" /><span className="text-xs font-bold text-brand-yellow uppercase tracking-wider">{t('games.my_best')}</span></div><div className="text-right"><div className="font-black font-mono text-xl text-brand-yellow">{userBestScore}</div>{isPvP && (<div className="text-[9px] font-black text-brand-yellow uppercase">{(selectedGameConfig.id === 'bee_chess' ? getTierInfo(userBestScore) : getGomokuTierInfo(userBestScore)).name}星</div>)}</div></div></div>)}</>
                        ) : (
                            <div className="flex flex-col flex-1 min-h-0"><div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 mb-4 pr-1 h-[250px]">{reviews.length === 0 ? (<div className="text-center text-neutral-400 py-8 text-sm flex flex-col items-center gap-2"><MessageSquare className="opacity-20" size={32}/>No reviews yet. Be the first!</div>) : (reviews.map((rev) => (<div key={rev.userId} className="bg-neutral-50 dark:bg-[#222] p-3 rounded-xl"><div className="flex justify-between items-start mb-2"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-neutral-200 overflow-hidden">{rev.avatarUrl ? <img src={rev.avatarUrl} className="w-full h-full object-cover"/> : <span className="flex items-center justify-center h-full w-full text-[10px]">👤</span>}</div><span className="text-xs font-bold dark:text-white">{rev.nickname}</span></div><div className="flex gap-0.5">{[...Array(5)].map((_, i) => (<Star key={i} size={10} className={i < rev.rating ? "fill-yellow-400 text-yellow-400" : "text-neutral-300 dark:text-neutral-600"} />))}</div></div><p className="text-sm text-neutral-600 dark:text-neutral-300 break-words leading-relaxed">{rev.comment}</p></div>)))}</div>{user ? (<div className="pt-3 border-t border-neutral-100 dark:border-[#333] shrink-0"><div className="flex gap-1 mb-2 justify-center">{[1, 2, 3, 4, 5].map((star) => (<button key={star} onClick={() => setUserRating(star)} className="p-1 hover:scale-110 transition-transform"><Star size={20} className={star <= userRating ? "fill-yellow-400 text-yellow-400" : "text-neutral-300 dark:text-neutral-600"} /></button>))}</div><div className="flex gap-2"><input type="text" value={userComment} onChange={(e) => setUserComment(e.target.value.slice(0, 140))} placeholder="Your review..." className="flex-1 bg-neutral-100 dark:bg-[#222] border-none rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 dark:text-white outline-none" onKeyDown={(e) => e.key === 'Enter' && handleSubmitReview()}/><button onClick={handleSubmitReview} disabled={!userComment.trim() || submittingReview} className="bg-yellow-400 text-black p-2 rounded-xl hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed"><Send size={18} /></button></div></div>) : (<div className="pt-4 border-t border-neutral-100 dark:border-[#333] text-center shrink-0"><button onClick={onLoginRequest} className="text-xs text-blue-500 hover:underline">Login to review</button></div>)}</div>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="hidden lg:flex lg:col-span-4 bg-white dark:bg-[#161616] rounded-[2.5rem] border border-neutral-200 dark:border-[#333] overflow-hidden flex-col shadow-xl lg:sticky lg:top-24 lg:h-[calc(100vh-120px)]">
                <div className="p-6 border-b border-neutral-100 dark:border-[#222] bg-neutral-50/50 dark:bg-[#1a1a1a]"><h3 className="font-black text-lg dark:text-white">{t('games.all_games')}</h3><p className="text-xs text-neutral-500 mt-1">{t('games.select_hint')}</p></div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">{GAMES.map(game => (<button key={game.id} onClick={() => setSelectedGameConfig(game)} className={`w-full p-3 rounded-2xl flex items-center gap-4 transition-all duration-300 group text-left border ${selectedGameConfig.id === game.id ? 'bg-brand-yellow text-black border-brand-yellow shadow-lg scale-[1.02]' : 'bg-white dark:bg-[#1e1e1e] border-transparent hover:bg-neutral-100 dark:hover:bg-[#252525] dark:text-gray-300'}`}><div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${selectedGameConfig.id === game.id ? 'bg-black/10' : 'bg-neutral-100 dark:bg-[#333]'}`}><game.icon size={24} className={selectedGameConfig.id === game.id ? 'text-black' : 'text-neutral-500 dark:text-gray-400'} /></div><div className="flex-1 min-w-0"><div className="font-bold truncate">{t(game.nameKey)}</div><div className="flex items-center gap-2 text-xs opacity-70"><span className="flex items-center gap-1"><Users size={10}/> {playerCounts[game.id] || 0}</span>{game.isHoneyGame && <span className="flex items-center gap-1 text-orange-600 dark:text-yellow-400 font-bold"><Zap size={10} className="fill-current"/> {t('games.earn_honey')}</span>}</div></div>{isRewardClaimed(game.id) ? (<div className="text-green-500" title="Reward Claimed"><CheckCircleIcon size={20} className="fill-green-500/20"/></div>) : selectedGameConfig.id === game.id && (<div className="w-2 h-2 rounded-full bg-black animate-pulse"></div>)}</button>))}</div>
            </div>
        </div>
      </div>
    </div>
